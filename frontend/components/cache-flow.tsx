...
export function CacheFlow() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      setCapabilities(caps); setConnection("online");
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (submission.current || clearing || connection !== "online") return;
  ...
  async function clear() {
    if (pending || clearing) return false;
  ...
  return <>
    ...
    <header className="site-header shell"><a href="#" className="wordmark" aria-label="Cache Flow home"><span className="brand-mark" aria-hidden="true"><svg viewBox="0 0 34 34" fill="none"><path d="M3 17h8c6 0 3-11 11-11h2a9 9 0 0 1 0 18H14m5-5-5 5 5 5" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /><circle cx="3" cy="17" r="2.5" fill="currentColor" /><circle className="reuse-node" cx="24" cy="6" r="3" /></svg></span>CACHE FLOW</a><nav aria-label="Main navigation"><a href="#lab">Query lab</a><a href="#ledger">Cache ledger</a><a href="#quality">Quality</a></nav><div className="header-tools"><ThemeToggle /><div className={`connection ${connection}`} role="status"><i />{connection === "starting" ? "Connecting" : online ? "API online" : "API offline"}</div></div></header>
    <main className="shell">
      ...
      <section id="lab" className="lab section-anchor"><div className="section-title"><div><p className="eyebrow">01 / SEND A SIGNAL</p><h2>Query lab</h2></div><span className="mode-tag">{capabilities ? capabilities.app_mode === "demo" ? "HOSTED DEMO" : "LOCAL MODE" : connection === "offline" ? "API OFFLINE" : "CONNECTING TO LAB"}</span></div>
        <form onSubmit={submit}><fieldset disabled={!online || pending || clearing} className="composer"><div className="prompt-side"><label className="eyebrow" htmlFor="prompt">YOUR QUESTION</label><textarea id="prompt" value={prompt} onChange={event => setPrompt(event.target.value)} required placeholder="What would you like to ask?" /><div className="examples"><span>TRY</span>{examples.map(example => <button type="button" className="example" key={example.label} onClick={() => setPrompt(example.question)}>{example.label} ↗</button>)}</div><div className="composer-bottom"><span className="subtle">{provider === "Demo" ? "Offline topic rules. No API key needed." : provider === "Ollama" ? "Uses the Ollama server configured on the backend." : "Credentials are configured on the backend. No keys enter this browser."}</span><button type="submit" className="send-button" disabled={!prompt.trim()}>{pending ? "Request in flight…" : "Trace request"}<span aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><path d="M2 19h6c5 0 2-14 7-14h6m-5-4 5 4-5 4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg></span></button></div></div>
          <div className="settings-side"><div className="setting-row"><label htmlFor="provider">Provider</label><select id="provider" value={provider} onChange={event => selectProvider(event.target.value as Provider)}>{capabilities ? capabilities.providers.map(item => <option key={item.name}>{item.name}</option>) : <option>Demo</option>}</select></div><div className="setting-row"><label htmlFor="model">Model</label><input id="model" className="mono" value={model} readOnly={provider === "Demo"} required={provider !== "Demo"} onChange={event => setModel(event.target.value)} placeholder="Backend default" /></div><div className="threshold-setting"><label htmlFor="threshold">Reuse threshold <output className="mono">{threshold.toFixed(2)}</output></label><input id="threshold" type="range" min="0" max="1" step="0.01" value={threshold} onChange={event => setThreshold(Number(event.target.value))} /><div className="range-caption"><span>More reuse</span><span>Closer match</span></div></div><div className="setting-row"><label htmlFor="ttl">TTL <span className="subtle">hours</span></label><input id="ttl" type="number" step="1" value={ttl} onChange={event => setTtl(event.target.value)} required className="mono ttl-input" /></div><span className="ttl-note">0 or negative = no expiration</span><label className="isolation"><input type="checkbox" checked={isolated} onChange={event => setIsolated(event.target.checked)} /><span>Isolate by provider + model<small>{isolated ? "Reuse stays within the selected model." : "Shared reuse across every provider and model."}</small></span></label></div>
        </fieldset></form>
      ...
      {result && !pending ? <Decision result={result} /> : !pending && !queryError && <div className="ready-strip"><span className="ready-cross">+</span><p>{online ? "Ready to trace your first request." : "Your next request starts here."}<small>Repeat the same question for exact reuse. Change the wording to test semantic matching.</small></p><span className="mono">REQUEST / {online ? "READY" : "WAITING"}</span></div>}
      ...
      <Ledger snapshot={snapshot} loading={loadingCache} clearing={clearing} error={cacheError} disabled={!online || pending} refresh={() => void refresh()} clear={clear} />
      {capabilities && <Quality key={capabilities.app_mode} defaultThreshold={capabilities.defaults.threshold} disabled={!online || pending || clearing} />}
