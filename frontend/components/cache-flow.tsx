...
export function CacheFlow() {
  ...
  return <>
    ...
    <main className="shell">
      ...
      {result && !pending ? <Decision result={result} /> : !pending && !queryError && <div className="ready-strip"><span className="ready-cross">+</span><p>{online && engine === "ready" ? "Ready to trace your first request." : "Your next request starts here."}<small>Repeat the same question for exact reuse. Change the wording to test semantic matching.</small></p><span className="mono">REQUEST / {online && engine === "ready" ? "READY" : "WAITING"}</span></div>}
      ...
      <Ledger snapshot={snapshot} loading={loadingCache} clearing={clearing} error={cacheError} disabled={!online || pending || engine !== "ready"} refresh={() => void refresh()} clear={clear} />
      {capabilities && <Quality key={capabilities.app_mode} defaultThreshold={capabilities.defaults.threshold} disabled={!online || pending || clearing || engine !== "ready"} />}
